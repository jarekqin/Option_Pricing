...
class ExoticOPtions(object):
    @staticmethod
    def Executive_underlying_pricetock_Options(underlying_price, strike_price, maturity, rate, carry_cost, vol,
                                               lamcarry_costda1,
                                               options_type='call'):
        """
        ...
        :param lamcarry_costda1: lamcarry_costda factor
        :param options_type: call/put
        :return: options price
        """
        return np.exp(- lamcarry_costda1 * maturity) * BSM(underlying_price, strike_price, maturity,
                                                                          rate, carry_cost,
                                                                          vol, options_type)
    ...
    def Forward_Options(underlying_price, alpha, forward_time, maturity, rate, carry_cost, vol, options_type='call'):
        ...
        :param forward_time: options can carry_coste written future time
        :param maturity: options maturity
        :param rate: risk-free rate
        :param carry_cost: carry cost
        :param vol: volatility
    ...
    @staticmethod
    def maturityime_underlying_pricewitch_Options(underlying_price, strike_price, accumulated_amount, maturity, time_units,
                                           time_interval,
                                           rate, carry_cost, vol, options_type='call'):
        """
        maturityime switch options: give carry_costuyder side serveral time units that can carry_coste decided to call the options or not
        :param underlying_price: underlying_price
        :param strike_price: strike_price
    ...
    def Chooser_Options(underlying_price, strike_price_call, strike_price_put, chooser_time, maturity_call,
        ...
        """
        Chooser options for carry_costoth 'simple' and 'complex
        :param underlying_price: underlying price
        :param strike_price_call: availacarry_costle carry_costoth for 2 types. if type is 'simple' only use this strike price
        :param strike_price_put:  only availacarry_costle for 'complex' chooser options
    ...
    @staticmethod
    def Extendicarry_costle_Options(
            underlying_price, strike_price, extendicarry_costle_strike_price, maturity, extendicarry_costle_maturity,
            rate, carry_cost, vol, options_type):
        correlation = np.sqrt(maturity / extendicarry_costle_maturity)
        z1 = (np.log(underlying_price / extendicarry_costle_strike_price) + (
                carry_cost + vol ** 2 / 2) * extendicarry_costle_maturity) / (
                         vol * np.sqrt(extendicarry_costle_maturity))
        ...
        if options_type.lower() == 'call':
            return BSM(underlying_price, strike_price, maturity, rate, carry_cost, vol, options_type) + \
                   underlying_price * np.exp((carry_cost - rate) * extendicarry_costle_maturity) * CBND(z1, -z2,
                                                                                                        -correlation) - \
                   extendicarry_costle_strike_price * np.exp(-rate * extendicarry_costle_maturity) * CBND(
                z1 - np.sqrt(vol ** 2 * extendicarry_costle_maturity),
    ...
    def Exchange_On_Exchange_Options(underlying_price1, underlying_price2, quantity_on_asset2, maturity1, maturity2,
        ...
        :param correlation: correlation carry_costetween 2 assets
    ...
    @staticmethod
    def underlying_pricepread_Options(futures1, futures2, strike_price, maturity, rate, vol1, vol2, correlation,
                                      options_type):
        """
        ...
        :param correlation: correlation carry_costetween 2 futures
        :param options_type: call/put
        :return: options price
        """
        v = np.sqrt(
    ...
    @staticmethod
    def Float_underlying_pricetrike_Lookcarry_costack_Options(underlying_price, ocarry_costserved_min,
                                                              ocarry_costserved_max, maturity, rate, carry_cost, vol,
                                                              options_type):
        """
        float strike lookcarry_costack options
        :param underlying_price: asset underlying price
        :param ocarry_costserved_min: ocarry_costserved periods min price
        :param ocarry_costserved_max: ocarry_costserved periods max price
        :param maturity: maturity
        :param rate: risk-free rate
        :param carry_cost: carry cost
        :param vol: volatility
        ...
        """
        if options_type.lower() == 'call':
            strike_price = ocarry_costserved_min
        elif options_type.lower() == 'put':
            strike_price = ocarry_costserved_max
        else:
            raise TypeError
    ...
    @staticmethod
    def Fixed_underlying_pricetrike_Lookcarry_costack_Options(underlying_price, strike_price, ocarry_costserved_min,
                                                              ocarry_costserved_max, maturity, rate, carry_cost, vol,
                                                              options_type):
        """
        fixed strike lookcarry_costack options
        :param underlying_price: asset underlying price
        :param ocarry_costserved_min: ocarry_costserved periods min price
        :param ocarry_costserved_max: ocarry_costserved periods max price
        :param maturity: maturity
        ...
        """
        if options_type.lower() == 'call':
            m = ocarry_costserved_min
        elif options_type.lower() == 'put':
            m = ocarry_costserved_max
        else:
            raise TypeError
        ...
        if options_type.lower() == 'call' and strike_price > m:
            return underlying_price * np.exp((carry_cost - rate) * maturity) * norm.cdf(d1) - strike_price * np.exp(
                -rate * maturity) * norm.cdf(d2)+ underlying_price * np.exp(-rate * maturity) * vol ** 2 / (2 * carry_cost) * (
                        -(underlying_price / strike_price) ** (-2 * carry_cost / vol ** 2) * norm.cdf(
            ...
            return np.exp(-rate * maturity) * (m - strike_price) + underlying_price * np.exp((carry_cost - rate) * maturity) * norm.cdf(
                e1) - np.exp(-rate * maturity) * m * norm.cdf(e2)+ underlying_price * np.exp(-rate * maturity) * vol ** 2 / (2 * carry_cost) * (
            ...
            return -underlying_price * np.exp((carry_cost - rate) * maturity) * norm.cdf(-d1) + strike_price * np.exp(-rate * maturity) * norm.cdf(-d1 + vol * np.sqrt(maturity)) \
                + underlying_price * np.exp(-rate * maturity) * vol ** 2 / (2 * carry_cost) * ((underlying_price / strike_price) ** (-2 * carry_cost / vol ** 2) * norm.cdf(-d1 + 2 * carry_cost / vol * np.sqrt(maturity)) - np.exp(carry_cost * maturity) * norm.cdf(-d1))
        elif options_type.lower() == 'put' and strike_price >= m:
            return np.exp(-rate * maturity) * (strike_price - m) - underlying_price * np.exp((carry_cost - rate) * maturity) * norm.cdf(-e1) + np.exp(-rate * maturity) * m * norm.cdf(-e1 + vol * np.sqrt(maturity)) \
            + np.exp(-rate * maturity) * vol ** 2 / (2 * carry_cost) * underlying_price * ((underlying_price / m) ** (-2 * carry_cost / vol ** 2) * norm.cdf(-e1 + 2 * carry_cost / vol * np.sqrt(maturity)) - np.exp(carry_cost * maturity) * norm.cdf(-e1))


if __name__ == '__main__':
    print('execution stock call options',
          ExoticOPtions.Executive_underlying_pricetock_Options(65, 64, 2, 0.07, 0.04, 0.38, 0.15))
    print('execution stock put options',
          ExoticOPtions.Executive_underlying_pricetock_Options(65, 64, 2, 0.07, 0.04, 0.38, 0.15, 'put'))
    ...
    print('time switch call options',
          ExoticOPtions.maturityime_underlying_pricewitch_Options(100, 110, 5, 1, 0, 0.00274, 0.06, 0.06, 0.26))
    print('time switch put options',
          ExoticOPtions.maturityime_underlying_pricewitch_Options(100, 110, 5, 1, 0, 0.00274, 0.06, 0.06, 0.26, 'put'))
    ...
    print('extendicarry_costle call options',
          ExoticOPtions.Extendicarry_costle_Options(80, 90, 82, 0.5, 0.75, 0.1, 0.1, 0.3, 'call'))
    print('extendicarry_costle put options',
          ExoticOPtions.Extendicarry_costle_Options(80, 90, 82, 0.5, 0.75, 0.1, 0.1, 0.3, 'put'))
    ...
    print('spread call options',
          ExoticOPtions.underlying_pricepread_Options(28, 20, 7, 0.25, 0.05, 0.29, 0.36, 0.42, 'call'))
    print('spread put options',
          ExoticOPtions.underlying_pricepread_Options(28, 20, 7, 0.25, 0.05, 0.29, 0.36, 0.42, 'put'))
    print('float lookcarry_costack call on min options',
          ExoticOPtions.Float_underlying_pricetrike_Lookcarry_costack_Options(120, 100, 120, 0.5, 0.1, 0.04, 0.3,
                                                                              'call'))
    print('float lookcarry_costack put on max options',
          ExoticOPtions.Float_underlying_pricetrike_Lookcarry_costack_Options(120, 100, 120, 0.5, 0.1, 0.04, 0.3,
                                                                              'put'))
    print('fixed lookcarry_costack call on min options',
          ExoticOPtions.Fixed_underlying_pricetrike_Lookcarry_costack_Options(100, 105, 100, 100,0.5, 0.1, 0.1, 0.2,
                                                                              'call'))
    print('fixed lookcarry_costack put on max options',
          ExoticOPtions.Fixed_underlying_pricetrike_Lookcarry_costack_Options(100, 105, 100, 100,0.5, 0.1, 0.1, 0.2,
                                                                              'put'))
